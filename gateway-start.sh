...
docker run -v $(pwd)/tyk.standalone.conf:/opt/tyk-gateway/tyk.conf -v $(pwd)/apps:/opt/tyk-gateway/apps -v $(pwd)/bundles:/opt/tyk-gateway/middleware/bundles -d --name tyk_gateway -p 8080:8080 --link tyk_redis:redis -e TYKLANG='-python' colinkuo/tyk-gateway-docker:v2.7.2-base
